...
<BODY>
	<FORM id="customerForm" name="customerForm" action="${pageContext.request.contextPath }/action_findByPage.action"
		method=post>
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：系统管理 &gt; 评估动作</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
										...
										<TABLE cellSpacing=0 cellPadding=2 border=0>
											<TBODY>
												<TR>
													<TD>动作名称：</TD>
													<TD>
														<INPUT class=textbox id=sChannel2 style="WIDTH: 80px" maxLength=50 name="action_name" value="${model.action_name}">
													</TD>
													<TD>
														<INPUT class=button id=sButton2 type="submit" value="筛选 " name=sButton2>
													</TD>
													<td>
														<input class=button type="button" value="新增评估动作" onclick="location.href='${pageContext.request.contextPath }/action_initAddNewActionUI.action'">
													</td>
												</TR>
											</TBODY>
										</TABLE>
									...
									<TD>
										<TABLE id=grid
											...
											<TBODY>
												...
												<c:forEach items="${page.beanList }" var="action">
												<TR
													style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
													<TD>${action.action_name }</TD>
													<TD>${action.action_muscle }</TD>
													<TD>${action.action_type }</TD>
													<TD>
														<a href="${pageContext.request.contextPath }/action_initDetail?action_id=${action.action_id}">详情</a>
														&nbsp;&nbsp;
														<a href="${pageContext.request.contextPath }/action_initEditUI.action?action_id=${action.action_id}">修改</a>
														&nbsp;&nbsp;
														<a href="${pageContext.request.contextPath }/action_delete?action_id=${action.action_id}" onclick="return window.confirm('确定删除吗？')">删除</a>
													</TD>
												</TR>
