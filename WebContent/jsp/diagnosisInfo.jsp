	...
	$(function(){
		...
		$.post(url2,function(data){
			$(data).each(function(){
				...
				if(actionName == this.action_name){
		...
		<div class="container-fluid">
			<div class="bg">
				<div class="Content-Main">
					<form action="${pageContext.request.contextPath }/diagnosis_save.action" method="post" class="form-userInfo" enctype="multipart/form-data">
						<h2>评估记录信息</h2>
						<p>
	                ...
	                <label>
	                    <span>评估动作：</span>
	                    <select class="sel" name="action.action_id" id="actionId">
							<option value="">--请选择--</option>
						</select>
	                </label>
					<label>
	                    <span>评估日期：</span>
	                    <input type="text" name="diagnosis_date" value="${tempDiagnosis.diagnosis_date }" placeholder="请填写治疗日期">
	                </label>
					<label>
	                    <span>iEMG（肌电积分值）：</span>
	                    <input type="text" name="diagnosis_iEMG" value="${tempDiagnosis.diagnosis_iEMG }" placeholder="请填写康复治疗时长">
	                </label>
					<label>
	                    <span>RMS（均方根值）：</span>
	                    <input type="text" name="diagnosis_RMS" value="${tempDiagnosis.diagnosis_RMS }" placeholder="请填写康复治疗得分">
	                </label>
					<label>
	                    <span>MPF（平均功率频率）：</span>
	                    <input type="text" name="diagnosis_MPF" value="${tempDiagnosis.diagnosis_MPF }" placeholder="请填写康复治疗得分">
	                </label>
	                <label>
	                    <span>MF（平均频率）：</span>
	                    <input type="text" name="diagnosis_MF" value="${tempDiagnosis.diagnosis_MF }" placeholder="请填写康复治疗得分">
	                </label>
					<label>
	                    <span>设备名称:</span>
